...
async function signIn() {
    ...
    try {
        ...
        await browser.waitUntil(async () => {
            return await pages('registrationPage').registrationComponent.input('password').isDisplayed();
        }, { timeout: 10000 });

        await pages('registrationPage').registrationComponent.input('password').setValue(inputData.password);
        await pages('registrationPage').registrationComponent.submitBtn('signIn').click();
