...
When('I set up a profile', async () => {
    ...
    await setupPage.setupComponent.initialSettings('board').setValue(valuesForFields.registrationBoard);
    ...
    await setupPage.setupComponent.initialSettings('listFirst').setValue(valuesForFields.registrationListFirst);
    await setupPage.setupComponent.initialSettings('listSecond').setValue(valuesForFields.registrationListSecond);
    await setupPage.setupComponent.initialSettings('listThird').setValue(valuesForFields.registrationListThird);
    await setupPage.setupComponent.submitBtn('next').click();

    await setupPage.setupComponent.initialSettings('cardFirst').waitForDisplayed({ timeout: 10000 });
    await setupPage.setupComponent.initialSettings('cardFirst').setValue(valuesForFields.registrationCardFirst);
    await setupPage.setupComponent.initialSettings('cardSecond').setValue(valuesForFields.registrationCardSecond);
    await setupPage.setupComponent.submitBtn('next').click();
